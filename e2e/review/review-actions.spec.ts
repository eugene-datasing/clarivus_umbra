 ...
 */
test.describe.fixme("Document Review Actions (PDF view)", () => {
  const reviewUrl = `/batches/${SEED.documents.mainCaseFile.batchId}/review/${SEED.documents.mainCaseFile.id}`;

  test.fixme("clicking Redact opens the ground selector", async ({ page }) => {
  ...
  test.fixme("shows detection-type filter tabs in the sidebar", async ({ page }) => {
    await page.goto(reviewUrl);
    await expect(page.locator("body")).toContainText(/all detection/i);
    await expect(page.locator("body")).toContainText(/personal/i);
  });

  test.fixme("shows the All Detections tab with a count", async ({ page }) => {
    await page.goto(reviewUrl);
  ...
  test.fixme("shows confidence percentages alongside detection rows", async ({ page }) => {
    await page.goto(reviewUrl);
    await expect(page.locator("body")).toContainText(/\d+%/);
  });

  test.fixme("shows LGOIMA s7 ground suggestions in the sidebar", async ({ page }) => {
    await page.goto(reviewUrl);
    await expect(page.locator("body")).toContainText(/s7\(2\)/i);
  });

  test.fixme("shows previous/next document navigation", async ({ page }) => {
    await page.goto(reviewUrl);
    const body = await page.locator("body").textContent();
    expect(/prev|next|←|→/i.test(body ?? "")).toBeTruthy();
  });

  test.fixme("PdfViewer toolbar exposes zoom controls", async ({ page }) => {
    await page.goto(reviewUrl);
  ...
  test.fixme("PdfToolbar renders showPreview toggle when content is wide enough for dual-panel", async ({ page }) => {
  ...
  test.fixme("clicking an overlay button highlights the corresponding sidebar row", async ({ page }) => {
    await page.goto(reviewUrl);
  ...
  test.fixme("page indicator updates as the reviewer scrolls", async ({ page }) => {
    await page.goto(reviewUrl);
    await expect(page.locator(".react-pdf__Page canvas").first()).toBeVisible({ timeout: 30_000 });
