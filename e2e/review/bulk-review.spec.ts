...
test.describe.fixme("Bulk Review", () => {
  ...
  test.fixme("shows entity action buttons (Apply to All / Review Each / Skip)", async ({ page }) => {
    await page.goto(bulkReviewUrl);
  ...
  test.fixme("shows confidence percentages for detections", async ({ page }) => {
    await page.goto(bulkReviewUrl);
    await expect(page.locator("body")).toContainText(/%/);
  });

  test.fixme("shows document source for each detection", async ({ page }) => {
    await page.goto(bulkReviewUrl);
