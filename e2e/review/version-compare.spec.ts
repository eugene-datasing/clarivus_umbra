...
test.describe.fixme("Version Comparison", () => {
  const compareUrl = `/batches/${SEED.cases.featherstonStreet.id}/review/${SEED.documents.mainCaseFile.id}/compare`;

  test.fixme("renders the version comparison page", async ({ page }) => {
   ...
   */
  test.fixme("shows empty state when no snapshots exist", async ({ page }) => {
    await page.goto(compareUrl);
    await expect(page.locator("body")).toContainText(/no snapshots available/i);
  });

  test.fixme("has a back to review link", async ({ page }) => {
    await page.goto(compareUrl);
    const backLink = page.getByRole("link", { name: /back to review/i });
    await expect(backLink).toBeVisible();
  });

  test.fixme("explains when snapshots are created", async ({ page }) => {
    await page.goto(compareUrl);
