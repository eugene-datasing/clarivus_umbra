...
**The April 2026 detection-coverage plan is complete as far as executable work goes.** Phases 3, 4, and 5 shipped. Phases 3.5 and 6 were investigated and parked with diagnosed mechanisms preserved for future work. Current canonical is `docs/bench-baselines/baseline-2026-04-23-post-phase-5-median-N10/` (suite F1 0.644). Future detection-coverage effort should scope a new plan informed by this one rather than extending it.
...
- **Phase 5 — label-adjacent detection via deterministic regex** (PR #36, commit on `phase-5-label-adjacent`): **merged**. New deterministic pass in `lib/pipeline/label-adjacent.ts` (~310 lines) + `lib/pipeline/__tests__/label-adjacent.test.ts` (51 unit tests). Dictionary of ~15 label families (DOB / address / phone / email / IRD / NHI / passport / driver licence / bank account / vehicle reg / employee-identifier / salary / GP with proper-noun-context guard / ICD-10 with code-shape guard) mapped to detection types + s7(2)(a). Regex structure: start-of-line or pipe-cell-boundary anchor + label alternation + separator (colon / pipe / dash / tab / **newline** — the DI tabular-extraction shape that splits label and value onto consecutive lines was the bug the directional bench caught) + up-to-80-char value capture. FP guards: `confidential` type allowlisted alongside `nhi` in the enabled-types filter because the toggle system has no "Confidential" toggle; prose-leading-marker and back-to-back-label guards prevent over-fire. Wired into `process.ts` alongside `detectPatterns`; output flows through existing bbox enrichment + `(page, type, text, posY_rounded)` dedup. Post-PR-5 N=10 canonical captured at `docs/bench-baselines/baseline-2026-04-23-post-phase-5-median-N10`; `CANONICAL` now points at that folder. Landed numbers vs the post-Phase-4 canonical:
  - **Suite aggregate F1 0.616 → 0.644 (+2.8pp)**. Within the predicted +2–5pp.
  - **Commercial pathway: 0.452 → 0.514 (+6.2pp)**. Within the predicted +3–8pp.
  - **Personal pathway: 0.758 → 0.781 (+2.3pp)**.
  - **Governance pathway: 0.337 → 0.368 (+3.1pp)** — welcome side-effect, not the contract. Phase 5's deliverable was personal / commercial; governance stays **anchored at 0.337** for Phase 6 planning regardless of where this canonical sits. The lift here comes from employee-identifier rows (`ADC-2284`, `ADC-0917`) and similar labelled `confidential` fields that the expected.json maps to the governance pathway.
  - **All five fixtures lifted**: B1 +3.0pp (0.485 → 0.515), B2 +4.2pp (0.513 → 0.555), A +2.6pp (0.431 → 0.457), C1 +5.2pp (0.407 → 0.459), B3 +1.4pp (0.779 → 0.793). No fixture regressed.
  - **Variance characterisation**: max per-fixture stddev 5.8pp (C1, still the highest-variance fixture); A's stddev tightened from 2.9pp to 1.9pp. Max observed low-side deviation from median: **B2 11.1pp** (min 0.444, median 0.555) — uncomfortably close to the 12pp gate, 0.9pp of headroom. Watch-item for future PRs touching the pipeline; if a CI run catches B2's low tail during ordinary AI non-determinism, the gate could trip without an actual regression. Not a blocker for Phase 5 itself (the PR's own CI run was green), but worth a note in Phase 6 review.
  - Issue #35 filed for multi-line-address value-capture follow-up (`27 Kowhai Street` / `47 Pohutukawa Terrace` caught without suburb/postcode because DI splits across lines). Priority low; not blocking.

**Parked (investigated, decision against shipping, branch preserved unmerged):**

- **Phase 3.5 — harassment-risk seeding investigation** (investigation complete, **parked** 2026-04-23). A three-iteration spike against six prompt variants (`scripts/spike-harassment-risk.ts`, PR #33) diagnosed the root cause: the AI over-generalises grievance context when a priority-rule-in-carve-out intervention tries to flip witness names to `harassment-risk`, and prompt engineering has a ceiling for role-distinguishing types in witness-statement-shaped documents. The strongest candidate (V5Narrow + V1, witnesses-only scope) passed cleanly on B3 but over-fired 60% of runs on B2's complainant/respondent despite the wording's explicit exclusion clause — the model ignored the carve-out. Expected governance lift from the best-available variant was ~0.35–0.38 (short of the 0.40 stretch target) and would have required a substantive `B2.expected.json` retune to avoid a scorer regression. Given the modest lift and the semantic scope commitment required on B2, **no production patch was shipped**. Governance stays at the post-Phase-4 median **0.337** for planning purposes. Full diagnosis in `docs/detection-coverage-phase-3-5-investigation.md`. Issue #32 closed parked; PR #33 closed parked.

- **Phase 6 — structured outputs migration** (investigation complete, **parked** 2026-04-24). Architectural migration from `response_format: json_object` to `response_format: json_schema` with `strict: true` was implemented on branch `phase-6-structured-outputs` and validated with N=10 canonical capture. The migration delivered genuine reliability wins (zero parse errors, zero refusals, zero max_tokens truncations across 150 pipeline invocations; `validateDetection()` shrinks from defensive coercion to thin mapping; type / ground / confidence guarantees enforced at the Azure API boundary) but N=10 median showed systematic recall regression on sentence-level detection types: free-frank +11 missing-count, legal-privilege +10, harassment-risk +9 across the bench corpus. Regression concentrated on B2 governance pathway (−26.1pp) and B1 governance (−7.1pp); suite F1 essentially flat (−0.1pp). **Mechanism**: strict-mode schema enforcement precision-favours on ambiguous-type sentence content — the pre-Phase-6 free-form JSON let the model commit to approximate types when uncertain (e.g. free-frank vs legal-privilege vs confidential on the same candid-advice sentence), strict mode makes it decline and skip the detection entirely. Architectural wins were prophylactic rather than addressing observed production failures. Decided against shipping; **future Phase 6 re-attempt should couple schema migration with prompt-side pathway-disambiguation work** (targeted worked examples per ambiguous-type sentence shape, not heuristic directives which over-reached in the path-2 iteration). Phase 6 branch preserved unmerged; instrumented attempt (migration code + N=10 canonical + 9 directional iterations) stashed on that branch. Follow-up scope captured in issue #38. Issue #34 stays open pending the re-attempt.

**Next up.** With both parked, the April detection-coverage plan is complete as far as executable work goes. Phase 5 shipped the last measurable improvement (suite F1 +2.8pp to 0.644). The two parked sub-phases have diagnosed mechanisms captured in the retrospective and follow-up issues (#32, #33 for Phase 3.5; #34, #38 for Phase 6) that a future detection-coverage plan can pick up. The current canonical is the post-Phase-5 median-N10 capture and stays the reference point until future work intentionally moves it.

**Outstanding issues:**
- Issue #38 — Phase 6 re-attempt: schema migration + pathway-disambiguation prompt work. Priority low. Future detection-coverage work; design brief captures the mechanism (strict-mode recall-for-precision trade on ambiguous-type sentence content) and the N=10-from-iteration-one requirement.
- Issue #35 — Label-adjacent: extend value capture across continuation lines for multi-line addresses. Priority low. Surfaced by the Phase 5 directional bench (B2's `27 Kowhai Street`, B3's `47 Pohutukawa Terrace` caught without suburb/postcode).
- Issue #34 — AI detection: occasional max_tokens cutoff on long composite prompts. Priority low. **Stays open pending Phase 6 re-attempt** — the Phase 6 architectural migration resolves it cleanly (150 pipeline invocations produced zero parse errors, zero refusals, zero truncations under the schema-enforced response format) but Phase 6 itself is parked. Interim workaround: bump `max_tokens` from 4096 to 8192 as the one-line mitigation if the production cutoff pattern recurs.
- Issue #32 — Phase 3.5 scope: AI under-seeding harassment-risk. **Closed parked** 2026-04-23 after the three-iteration spike documented in PR #33 (also closed parked). The AI's grievance-context over-generalisation makes prompt engineering an ineffective lever for this typing distinction.
...
**Status (2026-04-23): PARKED.** Investigated via a three-iteration prompt spike (`scripts/spike-harassment-risk.ts`, PR #33 — closed parked); decided against a production patch on the evidence below. Governance stays at the post-Phase-4 N=10 median **0.337**. Full report at `docs/detection-coverage-phase-3-5-investigation.md`.

### Summary

Phase 4's N=10 canonical showed `fromHarassmentRisk: 0` across every sample, meaning the AI's entity-propagation pipeline for governance was wired but never fired. Phase 3.5 set out to diagnose why and ship a prompt-tuning intervention that would move governance from 0.337 toward the 0.40 stretch target.

Three spike iterations against six prompt variants established:

- **Iteration 1** (baseline + V1–V5 × N=3 on B1) identified V5 (explicit priority rule appended to the council-officials carve-out) as the only variant producing any harassment-risk typing on named witnesses — at 1/3 rate on 4 of 5 witnesses. Confidence interval too wide to ship on N=3 alone.
- **Iteration 2** (V5 at N=10, V5+V1 and V5+V2 at N=5) pinned V5-alone's true rate at 10–20% (iteration 1's 1/3 was an unlucky-high sample) and showed **V5+V1 lifting the rate to 60–80% on B1 witnesses** on a five-run sample. Recommended for production patch.
- **Production directional bench on `phase-3-5-patch`** (V5+V1 applied verbatim to `lib/pipeline/ai-detect.ts`) revealed a catastrophic over-fire: B2 F1 dropped 15.3pp and the governance pathway regressed 8pp instead of lifting, because the rule wording ("witnesses AND complainant AND respondent are typed harassment-risk") activated broadly on every party in a grievance document, including non-witnesses.
- **Iteration 3** (V5Narrow + V1 at N=5 on B1, B2, B3) narrowed the rule's scope to witnesses only. Result: B3 over-fire cleanly eliminated (the rule correctly respects absence-of-explicit-Interviews-table as its discriminator); B1 witness seeding survived at 20–40% rate (Torres 40%, the other three witnesses 20%); **but B2 over-fire persisted at 60%** — the AI ignored the narrowed rule's explicit "complainant and respondent are NOT covered" clause and typed Ferguson/Kellogg/Torres as harassment-risk anyway.

### Root-cause diagnosis

The AI over-generalises grievance context. A priority rule that tells the model "witnesses in a grievance document are typed harassment-risk" is read as "names in a grievance-adjacent document are typed harassment-risk", regardless of role-based carve-outs the rule adds afterwards. The positive-rule sentence is structurally stronger than any exclusion that follows it. Prompt engineering has a ceiling for this class of role-distinguishing type decision; we cannot reliably make the model respect a role discriminator when the document-level context gives every name the same surface features.

This is not a wording fix. Three iterations of increasingly-specific exclusion language produced the same over-generalisation pattern on B2. Further narrowing (Path B in the iteration-3 report) would have tightened into literal-tabular-structure-only matching, which breaks the B1 witness seeding rate without solving the carve-out-respect problem.

### Decision against production patch

The best-available variant (V5Narrow + V1) projected a governance lift of approximately 0.35–0.38 — well short of the 0.40 stretch target. Shipping it would have required a substantive `B2.expected.json` retune to flip Ferguson / Kellogg / Torres / Mitchell from `personal-name` to `harassment-risk` to match the production behaviour, following the PR B Mitchell precedent. The retune is philosophically defensible under s7(2)(f)(ii) (private-complainant / private-respondent disputes ARE about retaliation protection) but the scope commitment is substantive and the payoff is below target.

Given the modest lift and the B2 scope commitment required, the decision was to park Phase 3.5 and accept governance at 0.337 rather than ship a marginal improvement that costs ground-truth stability.

### Architectural follow-up

Phase 6 (structured outputs migration) is expected to address the root cause architecturally. Schema-constrained type assignment — where the AI is forced to choose from an enum of valid types per ground rather than free-text decision — removes the "AI over-generalises grievance context into every name" failure mode because the constraint is enforced at the schema boundary, not at the prompt-adherence boundary. Whether that closes the governance gap is a separate experiment Phase 6 will measure directly; for now Phase 6 is queued as the next architectural attempt on this class of problem.

### Follow-up issues filed

- **#32** (original Phase 3.5 scope) — closed parked with a reference to this section.
- **#33** (investigation PR) — closed parked with a reference to this section.
- **Issue #34** — "AI detection: occasional max_tokens cutoff on long composite prompts" (priority/low). Two spike runs (iter-2 V5+V2 run2, iter-3 V5Narrow+V1 run4 on B3) returned empty JSON responses because detection completions exceeded the 4096-token `max_tokens` cap in `detectWithAI`. Both runs were dropped silently — no retry, no truncation detection. Recommended fix: bump `max_tokens` to 8192. Unrelated to the Phase 3.5 decision but surfaced by the spike work.

### Durable diagnostic artefacts

The spike infrastructure lives on the `phase-3-5-investigation` branch (PR #33, closed unmerged) — it is not on `main`. A future iteration on grievance-context typing, or on any role-distinguishing type decision, can branch from `phase-3-5-investigation` and extend the same harness with new variants rather than re-building the infrastructure. Notable reusable pieces:

- `scripts/spike-harassment-risk.ts` — three iterations of prompt-variance harness. Extracts any bench fixture via the production canonical-PDF path (issue #14 compliant), supports string-level prompt patches that assert their anchor text exists, runs N variants × N runs against cached extraction, emits per-run JSONs plus a per-party tally summary. DI extraction cache gitignored so re-runs of cached fixtures are Azure-OpenAI-only cost.
- `docs/detection-coverage-phase-3-5-investigation.md` — three-iteration diagnostic report. Contains the per-party tables, the pass/fail analysis against each iteration's criteria, and the architectural diagnosis that shaped the parking decision.
- `docs/spike-harassment-risk-2026-04-23/` — raw per-run detection JSONs for all 53 spike calls (iter-1 × 18, iter-2 × 20, iter-3 × 15). Useful as anchor data if a future spike needs to compare new-variant behaviour against these baselines.

---

## Phase 5 — Label-adjacent detection

**Status (2026-04-23): SHIPPED.** Merged on `phase-5-label-adjacent` (PR #36). Landed numbers captured at `docs/bench-baselines/baseline-2026-04-23-post-phase-5-median-N10/`. Suite F1 0.616 → 0.644 (+2.8pp). Commercial pathway +6.2pp; personal +2.3pp. Governance +3.1pp is a welcome side-effect (employee-identifier rows hit `confidential` type which maps to governance pathway); Phase 5's contract was personal / commercial only, governance stays anchored at **0.337** for Phase 6 planning.

**Two engineering bugs caught during the directional-bench sanity check** (the pause-before-N=10 pattern earned its keep):

1. **Newline-as-separator regex fix.** The initial implementation's separator regex treated only `:|–—\t` as label/value separators. Azure DI's tabular extraction emits labelled rows with **newline** between label and value (`Date of birth\n14 June 1983\nEmployee number\nADC-2284`), not colon-separated inline. First directional bench produced 0 label-adjacent matches on B1/B2/A/C1 because of this. Fix: add `\n` to the separator character class, plus a "value-itself-looks-like-a-label" guard for back-to-back-labels-without-value edge cases. Three tests added to lock this in (see `label-adjacent.test.ts` §"newline-separated labels").
2. **`enabledTypes`/confidential allowlist fix.** Second directional bench produced 9 B1 matches but zero `confidential` detections survived storage. Root cause: `getEnabledDetectionTypes()` returns a Set keyed on the toggle-map labels, and there is no "Confidential" toggle — `confidential` is a catch-all type the AI can always produce. The label-adjacent `enabledTypes` filter was excluding every `confidential`-typed entry (employee numbers, salary bands, ICD-10). Fix: allowlist `confidential` alongside `nhi` in the filter, matching `ai-detect.ts`'s `buildSystemPrompt` convention.

The sections below were preserved as-originally-drafted for historical context; the landed behaviour matches the spec with the bugs-caught-on-directional-bench fixes noted above.
...
**Status (2026-04-24): PARKED** after an instrumented attempt on branch `phase-6-structured-outputs` (not merged). Architectural migration achieved the reliability wins the plan set out to deliver — zero parse errors, zero refusals, zero `max_tokens` truncations across 150 pipeline invocations of the N=10 capture; `validateDetection()` thinned from defensive coercion to pass-through; type / ground / confidence guarantees enforced at the Azure API boundary — but N=10 median showed systematic recall regression on sentence-level detection types (free-frank, legal-privilege, harassment-risk) and on the governance pathway corpus-wide, concentrated on B2 (−26.1pp on its own governance pathway) and B1 (−7.1pp). Mechanism: strict-mode schema enforcement precision-favours on ambiguous-type sentence content — the pre-Phase-6 free-form JSON had let the model commit to approximate types when uncertain between `free-frank` / `legal-privilege` / `confidential` (or `commercial` / `negotiation` / `confidential`), strict mode makes it decline instead and skip the detection. Decided against shipping given no observed production failures to justify the F1 trade-offs (architectural wins were prophylactic) and given suite F1 essentially flat at −0.1pp. Future Phase 6 re-attempt should couple the schema migration with prompt-side pathway-disambiguation work (targeted worked examples per ambiguous-type sentence shape, not heuristic directives, which over-reached in this attempt's path-2 iteration). Branch `phase-6-structured-outputs` preserved unmerged; instrumented attempt (migration code + N=10 canonical at `docs/bench-baselines/baseline-2026-04-24-post-phase-6-median-N10/` + 9 directional iterations under `docs/bench-baselines/smoke-phase-6-v1/` through `v9/`) stashed on that branch. Follow-up scope in issue #38. Issue #34 stays open pending the re-attempt. Full narrative in `docs/detection-coverage-retrospective-2026-04.md` §"Phase 6 — structured outputs migration (parked)".

The sections below preserve the original Phase 6 spec for the future re-attempt to compare against.
