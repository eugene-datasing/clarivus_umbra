...
Phases 3, 4, and 5 are complete. **Phase 3.5 was investigated over a three-iteration prompt spike and parked** (see §"Phase 3.5 — Harassment-risk seeding investigation" below for the diagnosis; governance stays at the Phase 6 anchor 0.337 for planning purposes even though Phase 5's labelled-row coverage incidentally lifted it to 0.368 on the current canonical — see Phase 5 landing notes). What remains is Phase 6 (structured outputs migration). The executive summary and phase sections below describe the plan as originally drafted; use this status block as the orientation for what's done and where a fresh session should pick up.
...
- **Phase 3.5 — harassment-risk seeding investigation** (investigation complete, **parked** 2026-04-23). A three-iteration spike against six prompt variants (`scripts/spike-harassment-risk.ts`, PR #33) diagnosed the root cause: the AI over-generalises grievance context when a priority-rule-in-carve-out intervention tries to flip witness names to `harassment-risk`, and prompt engineering has a ceiling for role-distinguishing types in witness-statement-shaped documents. The strongest candidate (V5Narrow + V1, witnesses-only scope) passed cleanly on B3 but over-fired 60% of runs on B2's complainant/respondent despite the wording's explicit exclusion clause — the model ignored the carve-out. Expected governance lift from the best-available variant was ~0.35–0.38 (short of the 0.40 stretch target) and would have required a substantive `B2.expected.json` retune to avoid a scorer regression. Given the modest lift and the semantic scope commitment required on B2, **no production patch was shipped**. Governance stays at the Phase 6 anchor **0.337** for planning purposes. Phase 6 (structured outputs migration) likely addresses the root cause architecturally — schema-constrained type assignment removes the "AI over-generalises grievance context into every name" failure mode by eliminating the free-text type decision. Full diagnosis in `docs/detection-coverage-phase-3-5-investigation.md` (three-iteration report bracketed with `<!-- iter2-start/end -->` and `<!-- iter3-start/end -->` markers). Issue #32 closed parked; PR #33 closed parked.
- **Phase 6 — structured outputs migration** (queued, not started). Hardening track; AI output validated against a JSON schema so invalid grounds / types can't leak through. Expected to also close the Phase 3.5 root cause architecturally. Can slip without blocking any other phase.

**Next up:** Phase 6 (structured outputs migration). With Phase 5 shipped and the harassment-risk seeding investigation parked, Phase 6 is the last remaining piece of the April 2026 plan. Its main target is hardening the AI output against invalid-type / invalid-ground failure modes; its secondary opportunity is closing the "AI over-generalises grievance context" pattern that defeated Phase 3.5's prompt engineering, because schema-constrained enums enforce the type decision at the boundary rather than relying on prompt adherence.
...
- Issue #34 — AI detection: occasional max_tokens cutoff on long composite prompts. Priority low. Surfaced by the Phase 3.5 spike; fix is a one-line bump of `max_tokens` from 4096 to 8192 in `lib/pipeline/ai-detect.ts`.
- Issue #32 — Phase 3.5 scope: AI under-seeding harassment-risk. **Closed parked** 2026-04-23 after the three-iteration spike documented in PR #33 (also closed parked). The AI's grievance-context over-generalisation makes prompt engineering an ineffective lever for this typing distinction; Phase 6 structured-outputs migration is the next architectural attempt.
